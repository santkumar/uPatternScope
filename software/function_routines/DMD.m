...
classdef DMD < handle
    ...
    methods
        function dmd = DMD(varargin)
            ...
            addpath(fullfile(firstsep, libDir{1:end-1}, 'driverDMD\helperFunctions'));
            addpath(fullfile(firstsep, libDir{1:end-1}, 'driverDMD'));
        ...
        function definePattern(dmd) % 0x1A34
            ...
            idx             = 0;    % pattern index
            exposureTime    = 7200*1e4;  % exposure time in seconds (first value)
            clearAfter      = 0;    % clear pattern after exposure
